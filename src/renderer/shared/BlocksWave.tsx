...
export function BlocksWave({ className = "h-5 w-5" }: BlocksWaveProps): JSX.Element {
  ...
  if (reducedMotion === true) {
    return (
      <svg viewBox="0 0 24 24" className={`${className} shrink-0 text-capture`} aria-hidden="true">
  ...
  return (
    <span
      className={`inline-flex shrink-0 text-capture [&>svg]:h-full [&>svg]:w-full ${className}`}
      aria-hidden="true"
